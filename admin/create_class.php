<?php 
	...
	if ($rowCount > 0) {
		...
		if ($_SESSION['permission'] == 1) {
					$stmt=$pdo->prepare("INSERT INTO classroom (c_id , c_name , c_year , c_term , c_password , l_username , c_sec) VALUES (?,?,?,?,?,?,?)");
					...
					$stmt->bindParam(5,$_POST['c_password']);
												...
												$stmt=$pdo->prepare("INSERT INTO classroom (c_id , c_name , c_year , c_term , c_password , l_username , t_username,c_sec) VALUES (?,?,?,?,?,?,?,?)");
												$stmt->bindParam(1,$_POST['c_id']);
												$stmt->bindParam(2,$_POST['c_name']);
												$stmt->bindParam(3,$_POST['c_year']);
												$stmt->bindParam(4,$_POST['c_term']);
